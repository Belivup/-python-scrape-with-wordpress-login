...
def get_products_links(page):
    url = f'https://plugintheme.net/product-category/woocommerce-themes/page/{page}/'
    links = []
    r = s.get(url)
...
def parse_products(url):
    ...
    try:
        ...
        cat = ' '.join(scat).replace("Categories: ", "WooCommerce Themes > ")
    except AttributeError as err:
        cat = "WooCommerce Themes > WooCommerce Themes"
...
def save_csv(results):
    ...
    with open('Woo_Themes.csv', 'w') as f:
...
def page_len():
    r = s.get('https://plugintheme.net/product-category/woocommerce-themes/')
