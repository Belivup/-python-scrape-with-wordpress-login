...
def get_products_links(page):
    url = f'https://plugintheme.net/product-category/wordpress-plugins/page/{page}/'
    links = []
    r = s.get(url)
...
def parse_products(url):
    ...
    try:
        ...
        cat = ' '.join(scat).replace("Categories: ", "Wordpress Plugins > ")
    except AttributeError as err:
        cat = "Wordpress Plugins > Wordpress Plugins"
...
def page_len():
    r = s.get('https://plugintheme.net/product-category/wordpress-plugins/')
