...
def get_products_links(page):
    url = f'https://plugintheme.net/product-category/woocommerce-plugins/page/{page}/'
    links = []
    r = s.get(url)
...
def parse_products(url):
    ...
    try:
        ...
        cat = ' '.join(scat).replace("Categories: ", "WooCommerce Plugins > ")
    except AttributeError as err:
        cat = "WooCommerce Plugins > WooCommerce Plugins"
...
def save_csv(results):
    ...
    with open('Woo_Plugins.csv', 'w') as f:
...
def page_len():
    r = s.get('https://plugintheme.net/product-category/woocommerce-plugins/')
